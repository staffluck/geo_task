...
class TaskService:
    ...
    async def get_near_tasks(
        self, filters: TaskFilterByGeo, *, limit: int = 100, offset: int = 0
    ) -> list[TaskDTO]:
    ...
    async def get_task_detail(self, task_id: int) -> TaskDetail:
        return await self.task_uow.task_reader.get_task_detail(task_id)
    ...
    async def create_task(self, task_data: TaskCreate) -> TaskDTO:
        city_name = await self.task_uow.task.get_nearest_city_name(task_data.long, task_data.lat)
        task = Task.create(
